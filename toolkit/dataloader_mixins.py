...
class CaptionProcessingDTOMixin:
    ...
    def load_caption(self: 'FileItemDTO', caption_dict: Union[dict, None], **kwargs):
        enable_relation_captions = kwargs.get('enable_relation_captions', None)
        ...
        elif enable_relation_captions and '.'.join(os.path.basename(self.path).split('.')[:-1]) in enable_relation_captions:
            self.raw_caption = enable_relation_captions['.'.join(os.path.basename(self.path).split('.')[:-1])]['edit_prompt']
        else:
            # see if prompt file exists
...
class ImageProcessingDTOMixin:
    ...
    def load_and_process_image(
        ...
        try:
            swapped_bidir = False
            if self.is_box_analogy:
                img = Image.open(self.path)
                img = exif_transpose(img)

                control_img = Image.open(self.control_path)
                control_img = exif_transpose(control_img)

                img, swapped_bidir = self._create_box_analogy_image(control_img, img, do_transform=False)
            else:
                img = Image.open(self.path)
                img = exif_transpose(img)
        except Exception as e:
        ...
        # --- BOX ANALOGY TARGET IMAGE HANDLING ---
        # if hasattr(self, 'is_box_analogy') and self.is_box_analogy and self.has_control_image and hasattr(self, 'control_path') and self.control_path is not None:
        #     # Load the control image
        #     control_path = self.control_path[0] if isinstance(self.control_path, list) else self.control_path
        #     try:
        #         control_img = Image.open(control_path).convert('RGB')
        #         control_img = exif_transpose(control_img)
        #     except Exception as e:
        #         print_acc(f"Error: {e}")
        #         print_acc(f"Error loading control image: {control_path}")
        #         control_img = img  # fallback: use the target image itself
        #     # Use the method from ControlFileItemDTOMixin
        #     if hasattr(self, '_create_box_analogy_image'):
        #         self.tensor = self._create_box_analogy_image(control_img, img)
        #     else:
        #         # fallback: just use the image
        #         self.tensor = transforms.ToTensor()(img)
        # else:
        if self.augments is not None and len(self.augments) > 0:
            # do augmentations
        ...
        if not only_load_latents:
            if self.has_control_image:
                self.load_control_image(swapped_bidir=swapped_bidir)
...
class ControlFileItemDTOMixin:
    def __init__(self: 'FileItemDTO', *args, **kwargs):
        ...
        self.is_analogy = dataset_config.is_analogy
        self.is_box_analogy = dataset_config.is_box_analogy
        self.is_bidirectional_analogy = dataset_config.is_bidirectional_analogy
    ...
    def _process_single_image(self, img: Image) -> torch.Tensor:
    ...
    def _create_box_analogy_image(self, control_img: Image, target_img: Image, do_transform=True) -> torch.Tensor:
        """Create a 2x2 image for box analogy: [Control part 1, Control part 2; Control part 3, Target]"""
        # Ensure both images are the same size
        target_size = (512, 512) if not self.full_size_control_images else target_img.size #(self.crop_width, self.crop_height)
        target_img = target_img.resize(target_size, Image.BICUBIC)
        control_img = control_img.resize((target_size[0]*3, target_size[1]), Image.BICUBIC)
        ...
        swapped_bidir = self.is_bidirectional_analogy and random.random() < 0.5
        if swapped_bidir:
            control_parts[0], control_parts[1] = control_parts[1], control_parts[0]
            control_parts[2], target_img = target_img, control_parts[2]
        # Create 2x2 grid
        ...
        # Bottom row: Control part 3, Target
        bottom_row = Image.new('RGB', (2 * w, h))
        bottom_row.paste(control_parts[2].resize((w, h), Image.BICUBIC), (0, 0))
        bottom_row.paste(target_img.resize((w, h), Image.BICUBIC), (w, 0))

        # Combine rows
        combined_img = Image.new('RGB', (2 * w, 2 * h))
        combined_img.paste(top_row, (0, 0))
        combined_img.paste(bottom_row, (0, h))

        combined_img = combined_img.resize((target_size[0], target_size[1]), Image.BICUBIC) if not self.full_size_control_images else combined_img
        # combined_img = combined_img.resize((target_size[0], target_size[1]), Image.BICUBIC)

        # Convert to tensor
        if do_transform:
            transform = transforms.Compose([transforms.ToTensor()])
            return transform(combined_img), swapped_bidir
        else:
            return combined_img, swapped_bidir

    def load_control_image(self: 'FileItemDTO', swapped_bidir=False):
        control_tensors = []
        ...
        for control_path in control_path_list:
            ...
            if self.is_box_analogy:
                # For box analogy, we need both control and target images
                # The target image should be available as self.tensor (PIL Image)

                control_tensor = self.tensor.clone()
                # For box analogy control, copy bottom left to bottom right
                # Assuming control_tensor is [C, H, W] format
                h = control_tensor.size(1)
                w = control_tensor.size(2)

                # Copy bottom left quadrant to bottom right
                control_tensor[:, h//2:, w//2:] = control_tensor[:, h//2:, :w//2]
                if swapped_bidir:  # Copy upper left quadrant to upper right quadrant and the upper right quadrant to upper left quadrant
                    control_tensor[:, :h//2, :w//2], control_tensor[:, :h//2, w//2:] = control_tensor[:, :h//2, w//2:], control_tensor[:, :h//2, :w//2]
                control_tensors.append(control_tensor)
            elif self.is_analogy:
                # Split image into 3 equal parts on x axis
                w, h = img.size
                part_w = w // 3
                img_parts = [
                    img.crop((i * part_w, 0, (i + 1) * part_w, h))
                    for i in range(3)
                ]

                # Process each part and concatenate along channel dimension
                part_tensors = [self._process_single_image(part) for part in img_parts]
                tensor = torch.cat(part_tensors, dim=0)  # Concatenate along channel dimension

                control_tensors.append(tensor)
            else:
                tensor = self._process_single_image(img)
                control_tensors.append(tensor)
