...
class WandbLogger(EmptyLogger):
    def __init__(self, project: str, run_name: str | None, config: OrderedDict, resume_id: str | None) -> None:
    ...
    def start(self):
        ...
        run = wandb.init(project=self.project, name=self.run_name, config=self.config, id=self.run_id)
        self.run = run
...
def create_logger(logging_config: LoggingConfig, all_config: OrderedDict):
    if logging_config.use_wandb:
        ...
        resume_id = logging_config.resume_id
        return WandbLogger(project=project_name, run_name=run_name, config=all_config, resume_id=resume_id)
    else:
        return EmptyLogger()
