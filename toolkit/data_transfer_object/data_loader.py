...
from toolkit.dataloader_mixins import CaptionProcessingDTOMixin, ImageProcessingDTOMixin, \
    ControlFileItemDTOMixin, ClipImageFileItemDTOMixin, ArgBreakMixin, PoiFileItemDTOMixin, \
        MaskFileItemDTOMixin, AugmentationFileItemDTOMixin, \
...
class FileItemDTO(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.precomputed_filesize = kwargs.get('filesize', None)
        ...
        if use_db_entry:
            ...
            if self.dataset_config.fast_image_size: ## SLOW
            # original method is significantly faster, but some images are read sideways. Not sure why. Do slow method by default.
                ...
                if self.precomputed_filesize:
                    w, h = self.precomputed_filesize["width"], self.precomputed_filesize["height"]
                else:
                    img = exif_transpose(Image.open(self.path))
                    w, h = img.size
