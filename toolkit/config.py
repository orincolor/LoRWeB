...
def get_config(
    ...
    config = preprocess_config(config, name)
    ...
    def deep_update(base_dict, update_dict):
        for key, value in update_dict.items():
            if key in base_dict and isinstance(base_dict[key], dict) and isinstance(value, dict):
                ...
                deep_update(base_dict[key][0], value[0])
            else:
                base_dict[key] = value

    if kwargs and 'resume_id' in config['config']['process'][0]['logging']:
        del config['config']['process'][0]['logging']['resume_id']

    if kwargs:
        deep_update(config['config']['process'][0], kwargs['config']['process'][0])
        if 'name' in kwargs['config']:
            config['config']['name'] = kwargs['config']['name']
