...
from toolkit.models.AttLoRA import AttLoRAModule
...
class LoRASpecialNetwork(ToolkitNetworkMixin, LoRANetwork):
    ...
    def __init__(
        ...
        if self.network_type.lower() == "attlora":
            self.module_class = AttLoRAModule
            module_class = AttLoRAModule
                ...
                if module.__class__.__name__ in target_replace_modules:
                    for child_name, child_module in module.named_modules():
                        ...
                        if (is_linear or is_conv2d) and not skip:
                            ...
                            if self.network_type.lower() == "attlora":
                                module_kwargs["loras_num"] = self.network_config.network_kwargs["loras_num"]
                                module_kwargs["lora_keys_dim"] = self.network_config.network_kwargs["lora_keys_dim"]
                                module_kwargs["lora_heads"] = self.network_config.network_kwargs["lora_heads"]
                                module_kwargs["lora_softmax"] = self.network_config.network_kwargs["lora_softmax"]
                                module_kwargs["mixing_coeffs_type"] = self.network_config.network_kwargs["mixing_coeffs_type"]
                                module_kwargs["external_query"] = self.network_config.network_kwargs["external_query"]
                                module_kwargs["query_projection_type"] = self.network_config.network_kwargs["query_projection_type"]
                                module_kwargs["pooling_type"] = self.network_config.network_kwargs["pooling_type"]
                                module_kwargs["query_dim"] = self.network_config.network_kwargs.get("external_query_dim", None)
                                module_kwargs["query_mode"] = self.network_config.network_kwargs.get("query_mode", "aa'bb'")
                            ...
                            loras.append(lora)
                            if self.network_type.lower() == "attlora":
                                lora_shape_dict[lora_name] = [list(lora.lora_down.shape), list(lora.lora_up.shape), list(lora.lora_keys.shape)]
                            else:
