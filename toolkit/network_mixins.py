...
class ToolkitModuleMixin:
    ...
    def attlora_forward(self: Module, x, *args, **kwargs):
        # module dropout
        if self.module_dropout is not None and self.training:
            if torch.rand(1) < self.module_dropout:
                return 0.0  # added to original forward
        ...
        if self.external_query:
            query = kwargs["external_query"]
            if query.ndim == 2 or (query.ndim == 3 and query.shape[1] > 1):
                query = query.reshape(query.shape[0], 1, -1)
        else:
            # don't lora on the text part (add), try only lora on the attn img kontext
            query = x # x only from the context area

            if x.ndim == 2:
                query = query.reshape(query.shape[0], 1, -1)
            else:
                wtext = 512 if kwargs['wtext'] else 0
                if self.query_mode == "aa'bb'":
                    query = x
                elif self.query_mode == "caa'bb'":
                    query = x[:, wtext + (x.shape[1] - wtext)//2:, :]
                elif self.query_mode == "caa'b":
                    query = x[:, wtext + (x.shape[1] - wtext)//2:, :]
                    query = query[:, : 3*(query.shape[1]//4), :]
                elif self.query_mode == "caa'":
                    query = x[:, wtext + (x.shape[1] - wtext)//2:, :]
                    query = query[:, :query.shape[1]//2, :]
                else:
                    raise ValueError(f"Query mode {self.query_mode} not supported")

                if self.query_pooling == "avg":
                    query = query.mean(dim=1)
                elif self.query_pooling == "max":
                    query = query.max(dim=1).values
                else:
                    raise ValueError(f"Unknown pooling type: {self.query_pooling}")
                query = query.reshape(query.shape[0], 1, -1)

        batch_size, nqueries, _ = query.shape

        head_dim = self.lora_keys.shape[-1] // self.lora_heads

        query = self.att_lora_to_q(query) # [B x n_queries x inner_dim]
        query = query.view(batch_size, nqueries, self.lora_heads, head_dim)
        query = query.transpose(1, 2)  # [B, heads, nqueries, head_dim]

        # Prepare keys - lora_keys has shape [N, inner_dim]
        keys = self.lora_keys.view(-1, self.lora_heads, head_dim)  # [N, heads, head_dim]
        keys = keys.transpose(0, 1).unsqueeze(0)  # [1, heads, N, head_dim]
        keys = keys.expand(batch_size, -1, -1, -1)  # [B, heads, N, head_dim]

        N = self.lora_keys.shape[0]
        attn_scale = head_dim**-0.5

        if self.lora_softmax:
            values = torch.eye(N, N, dtype=query.dtype, device=query.device).expand(batch_size, self.lora_heads, -1 , -1)  # [batch_size, heads, N, N]

            mixing_coeffs = torch.nn.functional.scaled_dot_product_attention(
        ...
        # [batch_size, heads, nqueries, N] -> [batch_size, N]
        if self.mixing_coeffs_type == "sum":
            mixing_coeffs = mixing_coeffs.sum(dim=(1,2))
            mixing_coeffs = mixing_coeffs.clamp(min=(0 if self.lora_softmax else -1), max=1)
        elif self.mixing_coeffs_type == "mean":
            mixing_coeffs = mixing_coeffs.mean(dim=(1,2))
        else:
            raise ValueError(f"Unknown mixing coeffs type: {self.mixing_coeffs_type}")

        if isinstance(self.dropout, nn.Dropout) or isinstance(self.dropout, nn.Identity):
            x = self.dropout(x)

        if self.rank_dropout is not None and self.rank_dropout > 0 and self.training:
            raise NotImplementedError("Rank dropout not implemented")
        else:
            scale = self.scale

        if x.ndim == 3:
            lora_output = torch.einsum("b s i, n i r, b n, n r o -> b s o", x, self.lora_down, mixing_coeffs, self.lora_up)
        elif x.ndim == 2:
            lora_output = torch.einsum("b i, n i r, b n, n r o -> b o", x, self.lora_down, mixing_coeffs, self.lora_up)

        if self.lora_up_bias is not None:
            bias_output = (mixing_coeffs @ self.lora_up_bias)
            lora_output = lora_output + bias_output
    ...
    def forward(self: Module, x, *args, **kwargs):
        ...
        if 'external_query' in kwargs:
            tmp_kwargs.pop('external_query')
        if 'wtext' in kwargs:
            tmp_kwargs.pop('wtext')
        org_forwarded = self.org_forward(x, *args, **tmp_kwargs)
        ...
        try:
            ...
            if 'external_query' in kwargs and kwargs['external_query'] is not None:
                kwargs['external_query'] = kwargs['external_query'].to(self.lora_down.weight.dtype)
        except AttributeError:
            lora_input = x.to(self.lora_down.dtype)
            if 'external_query' in kwargs and kwargs['external_query'] is not None:
                kwargs['external_query'] = kwargs['external_query'].to(self.lora_down.dtype)

        if self.__class__.__name__ == "AttLoRAModule":
            lora_output = self.attlora_forward(lora_input, *args, **kwargs)
        else:
...
class ToolkitNetworkMixin:
    ...
    def get_keymap(self: Network, force_weight_mapping=False):
        ...
        if self.network_type.lower() == 'dora':
            if keymap is not None:
                ...
                keymap = new_keymap
        elif self.network_type.lower() == "attlora":
            pass
    ...
    def load_weights(self: Network, file, force_weight_mapping=False):
        ...
        for key, value in weights_sd.items():
            ...
            if self.peft_format:
                ...
                load_key = load_key.replace('$$lora_down', '.lora_down')
                load_key = load_key.replace('$$lora_up', '.lora_up')
                load_key = load_key.replace('$$heads_out_mat', '.heads_out_mat')
                load_key = load_key.replace('$$lora_keys', '.lora_keys')
                load_key = load_key.replace('$$att_lora_to_q$$', '.att_lora_to_q.')
                for i in range(10):
                    load_key = load_key.replace(f'att_lora_to_q.{i}$$', f'att_lora_to_q.{i}.')
                load_key = load_key.replace('.att_lora_to_q.pre_proj$$', '.att_lora_to_q.pre_proj.')
        ...
        if len(to_delete) > 0:
            raise ValueError(f"Missing keys: {to_delete}")
    ...
    def merge_in(self, merge_weight=1.0):
        if self.network_type.lower() == 'dora' or self.network_type.lower() == 'attlora':
            return
