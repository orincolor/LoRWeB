...
from toolkit.lorm import count_parameters, print_lorm_extract_details, \
...
from toolkit.config_modules import SaveConfig, LoggingConfig, SampleConfig, NetworkConfig, TrainConfig, ModelConfig, \
    GenerateImageConfig, EmbeddingConfig, DatasetConfig, preprocess_dataset_raw_config, GuidanceConfig, validate_configs
...
from extensions_built_in.flux_kontext import FluxKontextModel
...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def sample(self, step=None, is_first=False):
        ...
        for i in range(len(sample_config.prompts)):
            ...
            gen_img_config_list.append(GenerateImageConfig(
                ...
                is_analogy=sample_config.is_analogy,
                is_box_analogy=sample_config.is_box_analogy,
                **extra_args
            ))
    ...
    def clean_up_saves(self):
        ...
        if os.path.exists(self.save_root):
            ...
            def is_milestone_step(filename, milestone=500):
                try:
                    step_num = int(os.path.basename(filename).split('_')[-1].split('.')[0])
                    return (step_num % milestone) == 0
                except Exception as e:
                    print_acc(f"Error {e} while getting step number from {filename}")
                    return False

            # Remove milestone files from the removal lists
            safetensors_to_remove = [f for f in safetensors_to_remove if not is_milestone_step(f)]
            pt_files_to_remove = [f for f in pt_files_to_remove if not is_milestone_step(f)]
            directories_to_remove = [f for f in directories_to_remove if not is_milestone_step(f)]
            embeddings_to_remove = [f for f in embeddings_to_remove if not is_milestone_step(f)]
            critic_to_remove = [f for f in critic_to_remove if not is_milestone_step(f)]
    ...
    def end_step_hook(self):
        need_save = False  # custom condition
        if need_save:
            resume_id = self.logger.run_id
    ...
    def run(self):
        ...
        if 'network_kwargs' in self.get_conf('network'):
            network_kwargs = {
                "external_query": self.get_conf('network')['network_kwargs'].get("external_query", False),
                "query_mode": self.get_conf('network')['network_kwargs'].get("query_mode", "aa'bb'"),
                "external_query_model": self.get_conf('network')['network_kwargs'].get("external_query_model", None),
            }
        else:
            network_kwargs = {}
        ...
        if not self.is_fine_tuning:
            if self.network_config is not None:
                ...
                if network_kwargs.get('external_query', False):
                    if 'clip' in network_kwargs['external_query_model']:
                        network_kwargs['external_query_dim'] = (self.sd.image_encoder.config.projection_dim
                                                                if 'p' not in network_kwargs['query_mode']
                                                                else self.sd.image_encoder.config.hidden_size)
                    elif 'siglip2' in network_kwargs['external_query_model']:
                        network_kwargs['external_query_dim'] = (self.sd.image_encoder.config.hidden_size)
                        if 'p' in network_kwargs['query_mode']:
                            raise NotImplementedError(f"partial queries not implemented for siglip")
                    else:
                        raise NotImplementedError(f"External query model {network_kwargs['external_query_model']} not supported yet")
